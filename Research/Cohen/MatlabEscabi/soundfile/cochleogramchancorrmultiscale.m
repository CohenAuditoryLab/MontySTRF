%
%function [CorrDataMS]=cochleogramchancorrmultiscale(data,Fs,dX,f1,fN,Fm,fm1,dfm,Nms,OF,Norm,MSflag,dis,ATT)
%	
%	FILE NAME 	: COCHLEOGRAM CHAN CORR MULTI SCALE
%	DESCRIPTION : Computes the cochleogram channel correlation coefficient at
%                 multiple scales. The cochleogram is first decomposed into
...
if ~isstruct(data)
    [CochDataMS]=cochleogrammultiscale(data,Fs,dX,f1,fN,Fm,fm1,dfm,Nms,OF,Norm,MSflag,dis,ATT);
else 
    CochDataMS=data;
end
...
for l=1:size(Sms,3)
    for k=1:size(Sms,1)
        for m=1:size(Sms,1)
            ...
            N=(length(CochDataMS.MSFilter(l).H)-1)/2;
            L=size(Sms,2);
            Sk=Sms(k,N+1:L-N,l);
            Sm=Sms(m,N+1:L-N,l);
...
CorrDataMS.fml=CochDataMS.fml;
CorrDataMS.fmh=CochDataMS.fmh;
CorrDataMS.faxis=CochDataMS.faxis;