...
%
% (C) Monty Escabi, August 2006 (Last Edit Feb 2019)
%
function [Y,S,t,X]=mripple(f1,f2,RD,FM,App,ModType,M,Fs,NS,RP,DF)
...
if strcmp(ModType,'Log')
    ...
    S=zeros(NS,length(time(1:DF:end)));  %Much faster if matrix is allocated first
    for k=1:NS
        %Displaying Progress
        clc
...
elseif strcmp(ModType,'Lin')
    ...
    S=zeros(NS,length(time(1:DF:end)));  %Much if matrix is allocated first
    for k=1:NS
        %Displaying Progress
        clc
        disp(['Generating carrier: ' int2str(k) ' of ' int2str(NS)]);
        ...
        Sk=1+App/2*sin( 2*pi*RD*X(k) + 2*pi*FM*time +RP );
        Y=Y+Sk.*sin(2*pi*faxis(k).*time + 2*pi*rand );
        if ~isinf(DF)   %Reduces Memory Requirement
            S(k,:)=Sk(1:DF:length(Sk));
            t=time(1:DF:length(time));
