%
%function []=cochleogramblocked(fileheader,data,Fs,dX,f1,fN,Fm,OF,TB,Norm,dis,ATT,FilterType)
%	
%	FILE NAME 	: COCHLEOGRAM BLOCKED
...
%                 similar to COCHLEOGRAM but the data is blocked into
...
%                             cochleogram by removing the filter delays.
%
...
User=getenv('LOGNAME');
f=['!rm -rf /tmp/cochleogramtmp' User];
eval(f)
f=['!mkdir /tmp/cochleogramtmp' User];
eval(f)
...
for k=1:length(fc)
    ...
    fid1=fopen(['/tmp/cochleogramtmp' User '/tempS' int2str(k) '.bin'],'w');
    fid2=fopen(['/tmp/cochleogramtmp' User '/tempSc' int2str(k) '.bin'],'w');
    fwrite(fid1,S,'float');
...
while (n-1)*NB<length(S)
    ...
    clear St Sct
    for k=1:length(fc)
        fid1=fopen(['/tmp/cochleogramtmp' User '/tempS' int2str(k) '.bin'],'r');
        fid2=fopen(['/tmp/cochleogramtmp' User '/tempSc' int2str(k) '.bin'],'r');
    ...
    CochData.Filter=Filter;          %MAE, Oct 2016
    ...
    eval(['CochData' int2str(n) '=CochData;'])
    
    %Saving Blocked Data; use append to add successive blocks to the data file 
    if n==1
        save ([fileheader '_AGram'],['CochData'  int2str(n)]);
    else
        save ([fileheader '_AGram'],['CochData'  int2str(n)],'-append');
    end
