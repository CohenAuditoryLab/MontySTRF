...
%	DESCRIPTION : Loads cochleogram blocks of the sound specifiedn
...
for count=1:LB  %LB=Number of Blocks - itterate over all blocks
    ...
    %Loading Cochleogram for each Block
    load ([SoundFileName '_AGram'],['CochData' int2str(count)]);

    %Computing Correlation coefficients for each Block
    f=['CochData' int2str(count)];
    [CorrData]=cochleogramdynchancorr(eval(f),Fs,dX,f1,fN,Fm,OF,dT,Overlap,Norm);
