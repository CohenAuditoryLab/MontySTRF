%
% function [SoundCat]=cochleogramdyncorravemodelvaldata(Data,PCNum,GMNum,Coeff)
%   
%   FILE NAME   : AUDIOGRAM DYN CORR AVE MODEL VAL DATA
...
%                 be repeated for every single sound in each category. This
%                 code uses Coeff to find principal component scores.
%                             
    ...
    for SoundCounter=1:NSounds(CatCounter)  %Going through all sounds - pseudo jackknife for validation
       ...
       %Fitting data from each category to gaussian mixture model
        for count=1:NCat
            ...
            try
               Cat(count).GMModel = fitgmdist(score(index,1:PCNum),GMNum,'CovarianceType','diagonal','Options',statset('MaxIter',1000)); %Fitting Gaussian mixture dist to each category's principal component scores
               Cat(count).flag=1;
        ...
        SoundCat(CatCounter).Sound(SoundCounter).XScore=X.ValData/(Coeff');  
