%
%function [] = readtankcont2klusta(TankFileName,BlockNumber,f1,f2,Thresh,dT,SpikePhase,TBlock,ProbeFile)
%
...
%Reading Data in TBlock second blocks
L=floor(T/TBlock);
for k=1:L
   [data] = TDT2mat(TankFileName,Block,'T1',(k-1)*NBlock/Fs,'T2',(k*NBlock-1)/Fs,'Type',4,'Store','sWav');  %Only read the streams
   X=reshape(data.streams.sWav.data,1,numel(data.streams.sWav.data));
   fwrite(fid1,X,'single');
end
[data] = TDT2mat(TankFileName,Block,'T1',k*NBlock/Fs,'T2',0,'Type',4,'Store','sWav');      %Read last incomplete block
X=reshape(data.streams.sWav.data,1,numel(data.streams.sWav.data));
fwrite(fid1,X,'single');
