%
%function [] = readtankcontint2klusta(TankFileName,BlockNumber,UF,f1,f2,Thresh,SpikePhase,TBlock,ProbeFile)
%
...
if nargin<7 | ~exist('SpikePhase')
    SpikePhase='negative';
end
if nargin<8 | ~exist('TBlock')
   TBlock=30;
end
if nargin<9 | ~exist('ProbeFile')
...
%Reading Data in 30 second blocks, upsampling, and storing to file
L=floor(T/TBlock);
for k=1:L
    ...
    [data] = TDT2mat(TankFileName,Block,'T1',T1,'T2',T2,'Type',4);  %Only read the streams
    ...
    if k==1
        X=X(:,1:end-UF*10);                                         %Removing Edge samples
    else
        X=X(:,UF*10+1:end-UF*10);                                   %Removing Edge samples
    end
...
T1=(k-1)*TBlock-10;
[data] = TDT2mat(TankFileName,Block,'T1',T1,'T2',T,'Type',4);       %Read last incomplete block
X=data.streams.sWav.data;
