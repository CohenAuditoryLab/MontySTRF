...
%                     .NFFT     - FFT Size
%
%(C) Monty A. Escabi, February 11, 2014 (Edit Feb. 2016)
%
function [CalibData] = calibfirdirect(DirectData,f1,f2,L,ATT,NFFT,Disp)
...
Wf=(0:NFFT/2)/(NFFT/2)*pi;
[Hband] = bandpass(f1,f2,1500,Fs,40,'n');
[B,A] = invfreqz(H,Wf,0,L-length(Hband));
...
if strcmp(Disp,'y')
    ...
    Offset=10*log10((872161/Fs).^2);
    %Offset=20;                                  %NEED TO CHECK
    Xpre=conv(Y,CalibData.hinv);
    [Pxx,F]=psd(Xpre,NFFT/4,DirectData.Fs);
    plot(F,10*log10(Pxx/NFFT*2./Po.^2)+Offset,'k')
    title('Predicted Transfer Function after Calibration')
