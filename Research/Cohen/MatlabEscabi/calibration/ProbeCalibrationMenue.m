...
NFFT=1024*4;
    ...
    elseif ProbeSelect==3 & AcquireSelect==0 %Calibration mode 3
        ...
        if SpeakerSelect==1
            fid=fopen([TempDirectory 'spchan1.f32'],'wb')
            fwrite(fid,CalibData1.h1inv,'float');
            fclose(fid);
        elseif SpeakerSelect==2
            fid=fopen([TempDirectory 'spchan2.f32'],'wb')
            fwrite(fid,CalibData1.h1inv,'float');
            fclose(fid);
        end
    ...
    elseif ProbeSelect==3 &  AcquireSelect==1   %Validation Mode
        ...
        if SpeakerSelect==1
            ...
            %Copying Filter Impulse Responses to Data Directory
            copyfile([TempDirectory 'spchan1.f32'],[CalibDirectory])
        elseif SpeakerSelect==2
            fid=fopen([TempDirectory 'spchan2.f32'],'wb')
            fwrite(fid,CalibData2.hinv,'float');
            fclose(fid);
            ...
            %Copying Filter Impulse Responses to Data Directory
            copyfile([TempDirectory 'spchan2.f32'],[CalibDirectory])
        end
