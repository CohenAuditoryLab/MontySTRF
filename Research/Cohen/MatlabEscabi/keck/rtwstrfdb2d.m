%
%function [taxis,faxis,STRF2d1,STRF2d2,PP,Wo1,Wo2,No1,No2,SPLN]=rtwstrfdb2d(SpecFile
%,T1,T2,spet,Trig,Fss,SPL,MdB,ModType,Sound,NBlocks,sprtype)
%
%       FILE NAME       : RT WSTRF DB 2D
%       DESCRIPTION     : Real Time diagonal of 2nd order spectro-temporal
%			  receptive field
...
%
function [taxis,faxis,STRF2d1,STRF2d2,PP,Wo1,Wo2,No1,No2,SPLN]=rtwstrfdb2d(SpecFile,T1,T2,spet,Trig,Fss,SPL,MdB,ModType,Sound,NBlocks,sprtype)
...
if strcmp(ModType,'dB')
	...
	while ~feof(fid) & TrigCount<length(Trig)-1
		...
		for k=1:length(spettrig1)
			...
			if L < N2
				STRF1=STRF1+ (MdB*[S1(:,M-(N2-L-1):M) S2(:,1:L+N1)] - RMSP).^2;
			elseif L+N1 > M
				STRF1=STRF1+ (MdB*[S2(:,L-N2+1:M) S3(:,1:N1-M+L)] - RMSP).^2;
			else
				STRF1=STRF1+ (MdB*[S2(:,L-N2+1:L+N1)] - RMSP).^2;
			end
		...
		for k=1:length(spettrig2)
			...
			%Averaging Pre-Event Spectral Profiles
			if L < N1
				STRF2=STRF2+ (MdB*[S1(:,M-(N1-L-1):M) S2(:,1:L+N2)] - RMSP).^2;
			elseif L+N2 > M
				STRF2=STRF2+ (MdB*[S2(:,L-N1+1:M) S3(:,1:N2-M+L)] -RMSP).^2;
			else
				STRF2=STRF2+ (MdB*[S2(:,L-N1+1:L+N2)] - RMSP).^2;
			end
