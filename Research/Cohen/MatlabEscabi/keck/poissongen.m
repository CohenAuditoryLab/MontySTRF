...
%                     For Stationary or Non-Stationary Rate Function (
%                     L(T))
%
%                     Note: Refractory period is imposed at 1/Fsd resolution
%
%   L               : Lambda.  Can be a constant or a time varying waveform
%                     sampled at Fs
%                     Must satisfy:  L(T) > 0
...
%
% (C) Monty A. Escabi, Feb. 2005 (Edit Feb 2017)
%
...
% refractory period, tr, the spike rate for exponentially distributed
...
%
%Lmean=fsolve(@(x) 1E10*( (dTMin+1./x).*exp(-x*dTMin)-1/mean(L) ),[.5 500],optimoptions('fsolve','Display','off'));
%L=L/mean(L)*Lmean(1);
...
% M=intfft(L)*dt;
% M=M(1:length(L));
% M2=M(2:length(M))-M(1:length(M)-1);  %Computing m(t+T)-m(t)
M=L*dt; % Edit Feb 2017 (MAE)
...
dN=round(dT/1000*Fs);
index=find(X>0);                %Edit Feb 2017 (MAE)
