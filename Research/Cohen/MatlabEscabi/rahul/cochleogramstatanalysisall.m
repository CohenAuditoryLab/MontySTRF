%
%function [] = cochleogramstatanalysisall(METAData,dX,f1,fN,Fm,fm1,dfm,Nms,OF,Norm,MSflag,ATT,dT,Overlap,GDelay,dFm,Save)
%	
...
%
function [] = cochleogramstatanalysisall(METAData,dX,f1,fN,Fm,fm1,dfm,Nms,OF,Norm,MSflag,ATT,dT,Overlap,GDelay,dFm)
...
for k=1:length(METAData)
     ...
     %Analyzing Data from different segments
      if ~isnan(METAData(k).Segment1(1))
          ...
          cochleogramstatanalysis(filename,T1,T2,dX,f1,fN,Fm,fm1,dfm,Nms,OF,Norm,MSflag,ATT,dT,Overlap,GDelay,dFm,Save,'SEG1');
      end
      if ~isnan(METAData(k).Segment2(1))
          T1=METAData(k).Segment2(1);
          T2=METAData(k).Segment2(2);
          cochleogramstatanalysis(filename,T1,T2,dX,f1,fN,Fm,fm1,dfm,Nms,OF,Norm,MSflag,ATT,dT,Overlap,GDelay,dFm,Save,'SEG2');
      end
      if ~isnan(METAData(k).Segment3(1))
          T1=METAData(k).Segment3(1);
          T2=METAData(k).Segment3(2);
          cochleogramstatanalysis(filename,T1,T2,dX,f1,fN,Fm,fm1,dfm,Nms,OF,Norm,MSflag,ATT,dT,Overlap,GDelay,dFm,Save,'SEG3');
      end
      if ~isnan(METAData(k).Segment4(1))
          T1=METAData(k).Segment4(1);
          T2=METAData(k).Segment4(2);
          cochleogramstatanalysis(filename,T1,T2,dX,f1,fN,Fm,fm1,dfm,Nms,OF,Norm,MSflag,ATT,dT,Overlap,GDelay,dFm,Save,'SEG4');
      end
      if ~isnan(METAData(k).Soundscape1(1))
          T1=METAData(k).Soundscape1(1);
          T2=METAData(k).Soundscape1(2);
          cochleogramstatanalysis(filename,T1,T2,dX,f1,fN,Fm,fm1,dfm,Nms,OF,Norm,MSflag,ATT,dT,Overlap,GDelay,dFm,Save,'SNDSCP1');
      end
      if ~isnan(METAData(k).Soundscape2(1))
          T1=METAData(k).Soundscape2(1);
          T2=METAData(k).Soundscape2(2);
          cochleogramstatanalysis(filename,T1,T2,dX,f1,fN,Fm,fm1,dfm,Nms,OF,Norm,MSflag,ATT,dT,Overlap,GDelay,dFm,Save,'SNDSCP2');
      end
