...
%
% (C) Monty A. Escabi, October 2006. Revised by Chen, Mar. 2007 (Edit MAE
%     Feb. 2017)
%
function [GModel]=strfgaborfit(STRFs,STRF,taxis,faxis,N,theta,betaLB,betaUB)
...
if nargin<8
    betaUB=[50 50 500 2*pi 1 8 8 4 2*pi 5*max(max(abs(STRF)))];
end
...
[beta10,RESNORM1,RESIDUAL,EXITFLAG,OUTPUT,LAMBDA,J1]=lsqcurvefit('strfgabor1',beta,input,STRFs,betaLB,betaUB);

%Fitting separable STRF model to real data - beta from prior step is used
%for initial parameters
[beta1,RESNORM1,RESIDUAL,EXITFLAG,OUTPUT,LAMBDA,J1]=lsqcurvefit('strfgabor1',beta10,input,STRF,betaLB,betaUB);
...
beta20=[beta1 beta1];
[beta2,RESNORM2,RESIDUAL,EXITFLAG,OUTPUT,LAMBDA,J2]=lsqcurvefit('strfgabor2',beta20,input,STRF,betaLB2,betaUB2);
...
while MSE1>theta && n1<N
       ...
       [beta,RESNORM,RESIDUAL,EXITFLAG,OUTPUT,LAMBDA,J]=lsqcurvefit('strfgabor1',beta,input,STRF,betaLB,betaUB);
...
while MSE2>theta && n2<N
       ...
       beta(8)=rand*betaUB2(8);                             %Ripple density
       if beta(8)<1/betaUB2(7)
          beta(7)=1;
       else
          beta(7)=1/beta(8).*(1+(rand-0.5)*0.25);           %Spectral bandwidth
       end
       ...
       [beta,RESNORM,RESIDUAL,EXITFLAG,OUTPUT,LAMBDA,J]=lsqcurvefit('strfgabor2',beta,input,STRF,betaLB2,betaUB2);
